...
def plot_scan(axis, scan, origin):
    axis.plot(scan[0, :], scan[1, :], '.')
    axis.plot(origin.x, origin.y, '.')
    # for pt in scan_raw:
...
if __name__ == "__main__":
    ...
    #tf = icp.compute(ref, read, init_tf)

    cov = np.diag([1, 1, 0.1])
    tfs = icp_with_random_perturbation(icp, read, ref, init_tf, 100, cov)
    print(tfs)

    # print(tf)
    # trans = Position(tf[0, 2], tf[1, 2])

    fig = plt.figure()

    axis = fig.add_subplot(1, 1, 1)
    plot_scan(axis, ref, origin)
    plot_scan(axis, read, move)

    axis.plot(tfs[:, 0], tfs[:, 1], '.')
